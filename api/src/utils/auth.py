...
def compute_body_hash(body: str) -> str:
    ...
    Use this to produce the value sent in X-Body-SHA256 and included in the
    signed message. Pass an empty string for bodyless requests (GET, DELETE).
    """
...
def generate_signature(psk: str, method: str, uri: str, timestamp: str, body_hash: str = "") -> str:
    ...
    where BODY_HASH is hex SHA-256 of the request body (empty string for
    bodyless requests). Clients compute the hash and send it as X-Body-SHA256;
    the REST API Gateway authorizer reads it from headers since REQUEST
